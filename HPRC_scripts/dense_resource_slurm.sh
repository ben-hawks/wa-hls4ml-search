#!/bin/bash
#SBATCH --job-name=fermi_hls4ml_dense_resource
...
for (( CONFIG=START_CONFIG; CONFIG<END_CONFIG; CONFIG++ )); do
    srun -N 1 -n 1 -c 2 --overlap conda run --no-capture-output --name wa-hls4ml python3 "${REPO_DIR}/iter_manager_v2.py" \
        -f "${REPO_DIR}/dense_resource_models/dense_resource_batch_$CONFIG.json" \
        -o "${REPO_DIR}/output/dense_resource_run_vsynth_2024-2" \
        ...
        --vsynth > "logs/dense_resource_$CONFIG.log" &
done
wait
echo "Job completed."
